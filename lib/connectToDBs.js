...
function connectToDB(connectionInfo, reject){
    var db = new mongodb.Db(connectionInfo.db, new mongodb.Server("127.0.0.1", 27017), {safe: false});

    connectionInfo.status = CONNECTING;
    console.log('Connecting to "' + connectionInfo.db + '.' + connectionInfo.collection + '"....');
    db.open(dbOpenFunc.bind(null, connectionInfo, reject));
}
...
function dbOpenFunc(connectionInfo, reject, err, db){
    ...
    else if (connectionInfo.user && connectionInfo.pass){
        db.authenticate(connectionInfo.user, connectionInfo.pass, connectFunc.bind(null, connectionInfo, db, reject));
    }
    else{
