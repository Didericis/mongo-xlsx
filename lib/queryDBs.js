...
var writeResultsToXLSX = require('./writeResultsToXLSX');
...
function queryDBs(snowball){
    ...
    nextQueryFunc(snowball, insertedIDs, duplicateIDs, queryContainers, cursor, options, collections);
}

function nextQueryFunc(snowball, insertedIDs, duplicateIDs, queryContainers, cursor, options, collections){
    var queryContainer;
    ...
    if (queryContainers.length !== 0){
        ...
        cursor.nextObject(nextObjectFunc.bind(null, snowball, insertedIDs, duplicateIDs, queryContainers, queryContainer, cursor, options, collections));
    }
    else{
        ...
        writeResultsToXLSX(snowball, collections.resultCollection, 'output.xlsx');
    }
}

function nextObjectFunc(snowball, insertedIDs, duplicateIDs, queryContainers, queryContainer, cursor, options, collections, err, data){
